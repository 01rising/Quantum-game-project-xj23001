...
func get_team_name(player_id: int) -> String:
	if player_id >= 0 and player_id < relief_team_names.size():
		return relief_team_names[player_id]
...
func layout_ui():
	...
	if news_feed_label != null:
		news_feed_label.position = Vector2(24, 60)
		news_feed_label.size = Vector2(520, 28)
...
func update_status():
	if not game_finished and not quantum_active:
		status_label.text = "Mission Control: %s ready" % get_team_name(current_player)
...
func update_aid_loadout_ui():
	...
	aid_loadout_label.text = "Current Aid: %s carrying %s" % [
	...
	match event_id:
		0:
			...
			if escalate_specific_crisis(tile_id):
				show_news("Storm Surge: a Flood crisis has worsened.")
			else:
				show_news("Storm Surge warning issued, but no Flood crisis is active.")

		1:
			global_instability += 1
			instability_events_count += 1
			update_instability_ui()
			show_news("Infrastructure Collapse: global instability increased by 1.")
		...
		2:
			...
			show_news("Supply Drop: %s gets +1 bonus on their next correct aid match." % get_team_name(target_player))
...
func _on_change_aid_button_pressed():
	...
	status_label.text = "%s changed aid loadout to %s." % [
		get_team_name(current_player),
		player_aid_loadouts[current_player]
	]
